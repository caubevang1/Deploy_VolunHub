...
class EventRepository extends BaseRepository {
  ...
  #prepareFilter(filter) {
    ...
    // Chỉ copy những giá trị có tồn tại (tránh undefined làm hỏng query)
    ...
    // Xử lý lọc theo ngày (Chỉ xử lý nếu thực sự có truyền date)
    if (mongoFilter.date && typeof mongoFilter.date === "string") {
      const d = new Date(mongoFilter.date);
      if (!isNaN(d.getTime())) { // Kiểm tra ngày hợp lệ
        const nextD = new Date(d);
        nextD.setDate(nextD.getDate() + 1);
        mongoFilter.date = { $gte: d, $lt: nextD };
      } else {
        delete mongoFilter.date; // Xóa nếu ngày không hợp lệ
      }
    }
  ...
  /**
   * Lấy danh sách kèm số lượng người tham gia thực tế
   */
  ...
  async getEventsByManager(managerId) {
    // find() đã có transform ở BaseRepository
