...
const validateUserEntry = (name, birthday, phone) => {
  ...
  if (cleanName.split(" ").length < 2 || !NAME_REGEX.test(cleanName)) {
    return { error: "Họ tên phải từ 2 từ trở lên và không chứa ký tự đặc biệt." };
  }
  ...
  if (age < 15 || age > 60) return { error: `Tuổi (${age}) không hợp lệ (Yêu cầu 15-60).` };

  const cleanPhone = phone?.replace(/\s/g, "");
  if (cleanPhone && !PHONE_REGEX.test(cleanPhone)) return { error: "Số điện thoại không hợp lệ (10 số)." };
...
export const verifyAndRegister = async (req, res) => {
  try {
    ...
    await User.create({ email, name: validation.cleanName, username, birthday, password: hashedPassword, gender, phone: validation.cleanPhone, avatar });
...
export const updateProfile = async (req, res) => {
  try {
    ...
    const updateData = { name: validation.cleanName, birthday, gender, phone: validation.cleanPhone };
    if (req.file) {
    ...
    const updated = await User.findByIdAndUpdate(req.user._id, { $set: updateData }, { new: true }).select("-password");
...
export const sendResetOtp = async (req, res) => {
  try {
    const { email } = req.body;
    if (!(await User.findOne({ email }))) return res.status(404).json({ message: "Email không tồn tại." });
    const otp = generateOtp();
    await Otp.create({ email, otp, purpose: "RESET", expiresAt: new Date(Date.now() + 5 * 60 * 1000) });
...
export const changePassword = async (req, res) => {
  try {
    ...
    if (!(await bcrypt.compare(oldPassword, user.password))) return res.status(400).json({ message: "Mật khẩu cũ sai." });
...
export const register = async (req, res) => {
  try {
    const { username, email, password, name, birthday, gender, phone } = req.body;
    const validation = validateUserEntry(name, birthday, phone);
    if (validation.error) return res.status(400).json({ message: validation.error });

    const newUser = new User({
      username, email, password: await bcrypt.hash(password, 10),
      name: validation.cleanName, birthday, gender, phone: validation.cleanPhone,
