...
export const getEventDetail = async (req, res) => {
  try {
    const { id } = req.params;
    // Sử dụng Repository và populate createdBy để lấy thông tin người quản lý (name, phone)
    const event = await EventRepository.findById(id, null, "createdBy");

    if (!event) {
      return res.status(404).json({ message: "Không tìm thấy sự kiện" });
    }

    // Tính số lượng người tham gia thực tế từ RegistrationRepository
    ...
    eventData.currentParticipants = currentParticipants;
...
export const deleteEventByAdmin = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Xóa sự kiện và dọn dẹp dữ liệu thành công" });
  } catch (error) {
    res.status(500).json({ message: "Lỗi server", error: error.message });
  }
};
...
export const updateUserRole = async (req, res) => {
  try {
    ...
    if (!role) return res.status(400).json({ message: "Vui lòng cung cấp vai trò mới." });
    ...
    if (userIdToUpdate === adminId) {
      return res.status(400).json({ message: "Admin không thể tự thay đổi vai trò của chính mình." });
    }
    ...
    res.status(200).json({ message: "Cập nhật vai trò người dùng thành công.", user: u });
  } catch (error) {
    res.status(500).json({ message: "Lỗi server", error: error.message });
  }
};
...
export const exportUsers = async (req, res) => {
  try {
    ...
    sendExportResponse(res, normalizedUsers, "users-export", format, [
      "id", "name", "birthday", "gender", "phone", "email", "username", "role", "status", "points", "createdAt", "updatedAt"
    ]);
  } catch (error) {
    res.status(500).json({ message: "Lỗi server", error: error.message });
  }
...
export const exportEvents = async (req, res) => {
  try {
    ...
    sendExportResponse(res, normalizedEvents, "events-export", format, [
      "id", "name", "category", "startDate", "endDate", "location", "status", "points", "maxParticipants", "managerName", "managerEmail", "createdAt", "updatedAt"
    ]);
  } catch (error) {
    res.status(500).json({ message: "Lỗi server", error: error.message });
  }
...
export const exportVolunteers = async (req, res) => {
  try {
    ...
    sendExportResponse(res, normalizedVolunteers, "volunteers-export", format, [
      "id", "name", "email", "phone", "status", "points", "completedEvents", "createdAt", "updatedAt"
    ]);
  } catch (error) {
    res.status(500).json({ message: "Lỗi server", error: error.message });
  }
...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    res.status(200).json({ totalUsers, totalEvents, pendingEventsCount, approvedEventsCount, rejectedEventsCount, completedEventsCount });
  } catch (error) {
    res.status(500).json({ message: "Lỗi server", error: error.message });
  }
};
  ...
  try {
    ...
    const eventsWithMetrics = await Promise.all(
      events.map(async (event) => {
        const recentRegistrations = await RegistrationRepository.countDocuments({ event: event._id, createdAt: { $gte: cutoffDate } });
        const recentLikes = await EventActionRepository.countDocuments({ event: event._id, type: "LIKE", createdAt: { $gte: cutoffDate } });
        const recentShares = await EventActionRepository.countDocuments({ event: event._id, type: "SHARE", createdAt: { $gte: cutoffDate } });
        const trendingScore = recentRegistrations * 3 + recentLikes * 2 + recentShares * 5;
        return { ...event, recentRegistrations, recentLikes, recentShares, trendingScore };
      })
    );
    const trendingEvents = eventsWithMetrics.sort((a, b) => b.trendingScore - a.trendingScore).slice(0, 10);
...
export const getRecentActivity = async (req, res) => {
  try {
    const recentlyPublished = await EventRepository.find({ status: "approved", updatedAt: { $gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } }, null, { sort: { updatedAt: -1 }, limit: 5 }, "createdBy");
    const recentPosts = await PostRepository.find({ createdAt: { $gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } }, null, { sort: { createdAt: -1 }, limit: 10 }, "event author");
    const recentComments = await CommentRepository.find({ createdAt: { $gte: new Date(Date.now() - 24 * 60 * 60 * 1000) } }, null, { sort: { createdAt: -1 }, limit: 10 }, "event user");
    res.status(200).json({ recentlyPublished, recentPosts, recentComments });
  } catch (error) {
    res.status(500).json({ message: "Lỗi server", error: error.message });
  }
};
  ...
  try {
    ...
    const volunteersWithStats = await Promise.all(
      volunteers.map(async (volunteer) => {
        const completedEvents = await RegistrationRepository.countDocuments({ volunteer: volunteer._id, status: "completed" });
        return { ...volunteer, completedEvents };
      })
    );
    const sortedVolunteers = volunteersWithStats.sort((a, b) => b.points - a.points).map((volunteer, index) => ({
      ...volunteer, rank: index + 1,
      avatar: volunteer.avatar && !volunteer.avatar.startsWith("http") ? `http://localhost:5000${volunteer.avatar}` : volunteer.avatar,
    }));
  ...
  try {
    ...
    const managersWithStats = await Promise.all(
      managers.map(async (manager) => {
        ...
        const totalVolunteers = await RegistrationRepository.countDocuments({ event: { $in: eventIds }, status: { $in: ["approved", "completed"] } });
        const score = completedEvents * 10 + totalVolunteers;
        return { ...manager, totalEvents, approvedEvents, completedEvents, totalVolunteers, score };
      })
    );
    const sortedManagers = managersWithStats.sort((a, b) => b.score - a.score).map((manager, index) => ({
      ...manager, rank: index + 1,
      avatar: manager.avatar && !manager.avatar.startsWith("http") ? `http://localhost:5000${manager.avatar}` : manager.avatar,
    }));
